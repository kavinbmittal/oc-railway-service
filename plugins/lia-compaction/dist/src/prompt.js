 ...
 */
export function buildUserMessage(transcript, opts) {
    const parts = [];
    if (opts.previousSummary) {
        parts.push("This conversation was already compacted once. Here is the previous summary:\n\n" +
            "--- PREVIOUS SUMMARY ---\n" +
    ...
    if (opts.compressionRatio && opts.compressionRatio > 0) {
        ...
        parts.push(`Target compression: reduce to roughly ${pct}% of the original length while preserving all critical details.`);
    }
