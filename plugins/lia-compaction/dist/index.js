...
function resolveApiKey(configEnv, provider) {
    const envVarMap = {
...
function register(api) {
    ...
    const logger = {
        info: (...args) => api.logger.info?.(args.map(String).join(" ")),
        warn: (...args) => api.logger.warn?.(args.map(String).join(" ")),
        error: (...args) => api.logger.error?.(args.map(String).join(" ")),
    };
    if (!apiKey) {
        logger.error(`[lia-compaction] No API key for provider "${provider}" — compaction will fall back to OpenClaw built-in`);
    }
    ...
    import("openclaw/plugin-sdk/core")
        ...
        const registerFn = sdk.registerCompactionProvider ??
            null;
        if (typeof registerFn !== "function") {
            // Try the direct module path
            return import(
            // @ts-expect-error — path exists at runtime in OpenClaw v2026.4.5+
            "openclaw/dist/plugins/compaction-provider.js").then((mod) => {
                if (typeof mod.registerCompactionProvider === "function") {
                    ...
                    logger.error("[lia-compaction] registerCompactionProvider not found — is OpenClaw v2026.4.5+?");
                }
            });
        }
        registerFn({
...
function buildSummarizer(modelId, apiKey, logger) {
    return async (params) => {
        ...
        const userMessage = buildUserMessage(transcript, {
            ...
            identifierInstructions: params.summarizationInstructions?.identifierInstructions,
        });
        logger.info(`[lia-compaction] Summarizing ${messages.length} messages (${transcript.length} chars)`);
        ...
        logger.info(`[lia-compaction] Compaction complete (${text.length} chars summary, ${response.usage.input_tokens} in / ${response.usage.output_tokens} out)`);
        return text;
    };
}
const liaCompactionPlugin = {
    id: "lia-compaction",
    name: "Lia Compaction",
    description: "Q&A-preserving compaction — keeps conversation structure, decisions, commitments, and emotional context intact.",
    configSchema: {
        jsonSchema: {
            type: "object",
            properties: {
