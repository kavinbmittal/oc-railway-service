  ...
  <div className="bg-[#121214] border border-zinc-800 rounded-sm flex flex-col relative overflow-hidden">
   ...
   <form onSubmit={handleSubmit}>
    <div className="p-5 space-y-6">
     ...
     {/* 1. Experiment Name */}
     <div>
      <label className="block text-xs font-medium text-zinc-400 mb-2">Experiment Name</label>
      <input
       type="text"
       value={name}
     ...
     {/* 2. Hypothesis */}
     <div>
      <label className="block text-xs font-medium text-zinc-400 mb-2">Hypothesis</label>
      <textarea
       value={hypothesis}
     ...
     {primaryMetric && (
      <div className="mt-8 border border-dashed border-zinc-800 rounded-sm p-4 bg-zinc-900/10 relative overflow-hidden">
       ...
       <div className="grid grid-cols-2 gap-4 relative">
        ...
        {targetValue && (
         <div className="flex flex-col">
          <div className="text-xs font-medium text-zinc-400 mb-1">Target Value</div>
          <div className="text-sm font-medium text-cyan-400">{targetValue}%</div>
         </div>
        )}
       </div>
      </div>
    ...
    {/* Error */}
    {error && (
     <div className="mx-5 mb-4 border border-red-500/20 bg-red-500/5 rounded-sm px-4 py-3 text-sm text-red-400">
      {error}
     </div>
    )}
    ...
    <div className="p-5 border-t border-zinc-800 bg-[#121214] flex justify-end gap-3">
     {onClose && (
      <button
       type="button"
       onClick={onClose}
       className="px-4 py-2 rounded-md border border-zinc-800 bg-[#121214] text-sm font-medium text-zinc-300 hover:bg-zinc-800 transition-colors"
      >
       Cancel
      </button>
     )}
     <button
      ...
      className="px-4 py-2 rounded-md border border-cyan-500/50 bg-cyan-500/10 text-sm font-medium text-cyan-300 hover:bg-cyan-500/20 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
     >
