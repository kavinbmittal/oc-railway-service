...
export function CostTimeline({ entries = [] }) {
  ...
  return (
    <div className="border border-border divide-y divide-border">
      {/* Header */}
      <div className="grid grid-cols-[1fr_80px_80px_60px] gap-2 px-4 py-2 text-[10px] uppercase tracking-[0.16em] text-muted-foreground/60 font-medium">
        <span>Task</span>
        ...
        return (
          ...
          >
            ...
            <span className="text-right">
              {entry.type === "claude-code" ? (
                <span className="text-[10px] uppercase tracking-wider text-cyan-400 bg-cyan-500/10 px-1.5 py-0.5">
                  CC
                </span>
              ) : (
                <span className="text-[10px] uppercase tracking-wider text-muted-foreground bg-muted/50 px-1.5 py-0.5">
                  API
                </span>
              )}
            </span>
